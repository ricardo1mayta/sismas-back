...
cat config/deploy.after.sh
cat config/deploy.before.sh

mv -f app/devops/docker/Dockerfile $WORKSPACE/
mv -f app/target/application-v1.jar $WORKSPACE/
mv -f config/appspec.yml $WORKSPACE/
mv -f config/deploy.after.sh $WORKSPACE/
mv -f config/deploy.before.sh $WORKSPACE/
zip -r sigmaversion-test.zip application-v1.jar appspec.yml deploy.after.sh deploy.before.sh Dockerfile -x app -x config -x deploy -x Jenkinsfile -x backend.sh
